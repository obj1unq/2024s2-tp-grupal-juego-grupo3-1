...
object superTablero {
  ...
  method tieneQueFinalizarJuego() = self.condicionFinalOlvidasteObjetoImportante() or reloj.seQuedoSinTiempo()
  
  method condicionFinalOlvidasteObjetoImportante() = mapaActual.tieneObjetoImportante() and self.teOlvidasteObjetoImportante()
  ...
  method finalSiSeQuedoSinTiempo() {
    if (reloj.seQuedoSinTiempo()) self.ejecutarFinal(finDeJuegoSinTiempo)
  }
  
  method finalSiGanaste() {
    if (self.ganaste()) self.ejecutarFinal(finDeJuegoGano)
  }
  ...
  method teOlvidasteObjetoImportante() = not objetosRecogidos.contains(
    mapaActual.objetoImportante()
  )
  
  method ganaste() = self.recogioTodosLosObjetosimportantes() and (not reloj.seQuedoSinTiempo())
  
  method recogioTodosLosObjetosimportantes() = objetosImportantes.all(
    { obj => objetosRecogidos.contains(obj) }
  )
  
  method sePuedeTrasladarElAuto() = self.cosasCon(auto).any(
    { objeto => objeto.meTraslada() }
  )
  
  method hayMetaEnPosicionDelAuto() = self.cosasCon(auto).any(
    { elem => elem.esMeta() }
  )
  
  method cosasCon(obj) = game.colliders(obj)
  
  method hayAlgoAgarrableCon(obj) = self.cosasCon(obj).any(
    { obj => obj.esAgarrable() }
  )
  ...
  method estaDentroDeLosLimites(position) = position.x().between(
    0,
    game.width() - 1
  ) and position.y().between(0, game.height() - 3)
  
  method haySolidoEn(position) = self.cosasEnLaPosicion(position).any(
    { cosa => cosa.solida() }
  )
  
  method cosasEnLaPosicion(position) = game.getObjectsIn(position)
  
  method estaEnElTablero(unaCosa) = game.allVisuals().contains(unaCosa)
} // Vacío
